...
const ResultsDashboard: React.FC<Props> = ({ projections, inputs }) => {
  ...
  // LOGICA DI AUDIT ESPLICITA (ANNO 1) - FORMULA LINEARE (1 + i - c)
  const audit = (() => {
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <div className={`p-6 rounded-[2rem] border transition-all ${inputs.isCampaignPeriod ? 'bg-amber-50 border-amber-200 shadow-lg shadow-amber-200/20' : 'bg-white border-slate-200 opacity-60'}`}>
          ...
          <p className="text-[9px] font-black text-amber-600/60 uppercase mt-2 tracking-tighter">Incentivi Campagna Attivi</p>
        </div>
        ...
        <div className="bg-[#003399] p-6 rounded-[2rem] border border-blue-900 shadow-xl shadow-blue-900/10">
          ...
          <p className="text-[9px] font-black text-blue-200/50 uppercase mt-2 tracking-tighter">Quota annua lorda</p>
        </div>
      </div>
      ...
      <div className="bg-white border-4 border-slate-900 rounded-[3rem] overflow-hidden shadow-2xl relative">
        <div className="absolute top-12 right-12 w-24 h-24 border-8 border-emerald-500/10 rounded-full flex items-center justify-center -rotate-12 pointer-events-none">
           <span className="text-[10px] font-black text-emerald-500/30 uppercase text-center leading-none">AUDIT<br/>CERTIFICATO</span>
        </div>
        
        <div className="bg-slate-900 p-8 text-white flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center text-blue-400">
               <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04m17.236 0a11.92 11.92 0 00-1.842-5.533L12 2.944l-7.394-1.453a11.92 11.92 0 00-1.842 5.533M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            </div>
            <div>
              <h3 className="text-2xl font-black tracking-tighter uppercase">Audit Trasparenza Costi</h3>
              <p className="text-[10px] font-black text-blue-300 uppercase tracking-[0.3em]">Versione Blindata e Certificata - Anno 1</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="bg-white/10 px-4 py-2 rounded-2xl border border-white/20 text-center">
              ...
              <span className="text-emerald-400 font-black text-xs uppercase">Lineare (i - c)</span>
            </div>
        ...
        <div className="p-10">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-12">
            ...
            {/* STEP 2: IL CUORE DEL CALCOLO COSTI */}
            <div className="space-y-6">
               <div className="flex items-center gap-4 mb-2">
          ...
          <div className="mt-12 bg-slate-900 rounded-[2.5rem] p-10 text-white relative shadow-2xl overflow-hidden">
             ...
             <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative z-10">
                <div className="text-center md:text-left">
                  ...
                  <p className="text-[9px] text-slate-500 mt-2">Comprende capitalizzazione netta + bonus ricorrenti</p>
                </div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mt-0.5">Scenari di crescita basati su rendimenti stimati</p>
          </div>
          <div className="flex gap-4 p-2 bg-slate-50 rounded-2xl border border-slate-100">
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-500"></div>
              <span className="text-[9px] font-black text-slate-500 uppercase">GS</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-blue-500"></div>
              <span className="text-[9px] font-black text-slate-500 uppercase">UL</span>
            </div>
          </div>
        </div>
        ...
        <div className="h-[350px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={projections} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <ReferenceLine x={inputs.horizonYears} stroke="#94a3b8" strokeDasharray="4 4" label={{ position: 'top', value: 'TARGET', fill: '#64748b', fontSize: 9, fontWeight: '900' }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>

      {/* SEZIONE TUTELA PATRIMONIALE */}
      <EstateProtection />
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm relative">
        ...
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <div>
            ...
            <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">Verifica puntuale di ogni anno di simulazione</p>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-[10px]">
            <thead className="bg-slate-50/80 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 font-black text-slate-400 uppercase text-[9px]">Anno</th>
                <th className="px-4 py-4 font-black text-emerald-600 uppercase text-right text-[9px]">Lordo GS</th>
                <th className="px-4 py-4 font-black text-blue-600 uppercase text-right text-[9px]">Lordo UL</th>
                <th className="px-4 py-4 font-black text-slate-800 uppercase text-right text-[9px]">Tot. Lordo</th>
                <th className="px-4 py-4 font-black text-red-400 uppercase text-right text-[9px]">Penale %</th>
                <th className="px-6 py-4 font-black text-red-600 uppercase text-right bg-red-50/30 text-[9px]">Val. Riscatto</th>
              </tr>
            </thead>
